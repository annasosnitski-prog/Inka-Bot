...
//   [ТАТУ] / [ПРИЁМ] — ставит МАСТЕР ВРУЧНУЮ или через синк Дневника
...
//   [ОКНО] / [ЧАТ] — единственные теги, которые бот вообще может
...
//     занятости — это конкретная бронь, не Дневник-сессия. Названия
//     тегов подобраны удобными для голосового ввода (см. SlotTag ниже);
//     старые WALKIN/ONLINE/КОНС/ВИДЕО остаются синонимами в командах бота
//     (lib/addSlotParser.ts), но не в самих тегах календаря. [ЧАТ] — вторая
//     версия тега [ВИДЕО] (2026-08-10): тег назывался "видео" и вводил
//     клиентов в заблуждение, будто это видеозвонок/съёмка — на деле это
//     консультация в переписке. "ВИДЕО" осталась только как распознаваемое
//     слово при вводе команды, реальный тег в календаре теперь [ЧАТ].
//     СУЩЕСТВУЮЩИЕ события с тегом [ВИДЕО] в календаре бот больше не
//     узнает — их нужно переименовать вручную (заменить "[ВИДЕО]" на
//     "[ЧАТ]" в названии события), иначе такой слот выпадает из подбора.
// Иными словами: ТАТУ/ПРИЁМ = "я лично веду этого клиента", ОКНО/ЧАТ
...
// файла. [ТАТУ]/[ПРИЁМ] сюда входят как тип для Дневник-событий (день-блок,
// /расписание, /закрой по семье), но НЕ как то, что бот предлагает клиенту.
// Названия тегов подобраны так, чтобы быть удобными для голосового ввода
// (по просьбе Ани — старые WALKIN/ONLINE плохо распознаются диктовкой на
...
// сделал бот — независимо от тега [ТАТУ]/[ПРИЁМ]/[ЧАТ]/[ОКНО] — это
...
export function formatSlotForDisplay(slot: AvailableSlot): string {
  ...
  // Пометка формата ("(по переписке)"/"(в студии)"/"(walk-in)") — чтобы
  // клиент понимал, что выбирает, когда в одном списке разные виды слотов.
...
//      блокирует ТОЛЬКО указанное окно, только консультации (ПРИЁМ+ЧАТ).
//   4. /закрой тату короче 4ч — блокирует ТОЛЬКО это окно времени, ТОЛЬКО
//      для тату-семьи (ТАТУ+ОКНО).
...
// и порогом ОКНО (см. lib/telegram.ts): маленькая работа = короче этого.
export const MAX_CONSULTATION_HOURS = 2;
// Минимальный запас перед ОКНО/ЧАТ-бронью — бот не предлагает клиенту
...
export interface DayBlockInfo {
  ...
  blockedConsultationIntervals: Array<{ startMs: number; endMs: number }>; // /закрой конс — только это окно, только ПРИЁМ+ЧАТ
  blockedTattooIntervals: Array<{ startMs: number; endMs: number }>; // /закрой тату <4ч — только это окно, только ТАТУ+ОКНО
}
...
export async function getAvailableSlots(type: SlotType, maxResults = 3): Promise<AvailableSlot[]> {
  ...
  // ненадёжно работает с квадратными скобками [ТАТУ]/[ПРИЁМ] (это
...
export async function bookSlot(
  ...
  // Маркер выбирается по ТЕГУ самого события: [ЧАТ] → "КОНС ЧАТ",
  // очная [ПРИЁМ] → "КОНС ЗАПИСЬ", [ТАТУ]/[ОКНО] → "ОЖИДАЕТ ПРЕДОПЛАТЫ".
...
// В отличие от getAvailableSlots (только свободные [ТАТУ]/[ПРИЁМ]),
...
// [ТАТУ]/[ПРИЁМ] (чтобы getSchedule определил тип) и содержит "ЗАНЯТО"
