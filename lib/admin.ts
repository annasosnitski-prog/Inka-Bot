...
//   /добавить <тег> <день> <время> — открыть новый слот в календаре
//   /закрой <тату|приём> <дата> <время> [имя] — заблокировать время
//   /удали <тату|приём> <дата> [имя] — снять запись из календаря
...
//
// Ограничение по длине — не просто эвристика: без него любое длинное
// сообщение, где слово "запись"/"расписание" встретилось ГДЕ УГОДНО
// внутри (например текст для перевода: "...когда запись несколько раз
...
export function isScheduleRequest(text: string): boolean {
  const t = text.trim();
  if (t.length > SCHEDULE_REQUEST_MAX_LENGTH) return false;
  return /запис|расписан|календар/i.test(t);
}
